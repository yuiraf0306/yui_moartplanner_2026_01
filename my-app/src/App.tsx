...
const RafayelPlannerTab = ({ user, appFrame }) => {
                  ...
                  return <div key={i} ref={el=>dayRefs.current[dk]=el} onClick={()=>goToDay(d)} className={`snap-center shrink-0 w-[80vw] sm:w-[300px] h-full rounded-[2rem] border overflow-hidden flex flex-col backdrop-blur-md transition-all ${isT?'border-pink-500/40 bg-purple-900/40':'border-purple-500/10 bg-[#1e1b4b]/40'} ${isS?'ring-2 ring-purple-400/50 scale-[1.02]':''}`}>
                      <div className="p-4 border-b border-white/5 bg-black/10 flex justify-between items-center">
                          ...
                          <div className="flex gap-2">
                             {mood && <span className="text-xl filter drop-shadow-sm">{mood.emoji}</span>}
                             <div className="p-2 rounded-full bg-white/5 text-white/50"><ArrowUpRight size={16}/></div>
                          </div>
                      </div>
          ...
          <div className="h-full flex flex-col pb-6 animate-in slide-in-from-bottom-5 duration-200 relative">
              ...
              {dayTab === 'tasks' ? (
                  ...
                  <div className="flex-1 relative rounded-3xl overflow-hidden mx-1 shadow-2xl kraft-paper transition-all pointer-events-auto flex flex-col pb-6">
                      <div className="w-full h-full overflow-y-auto custom-scrollbar p-6 flex flex-col pb-20">
  ...
  return (
    <div className="h-full flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 px-4 pb-4 overflow-hidden relative z-10">{loading?<div className="h-full flex items-center justify-center text-purple-200"><div className="animate-spin-slow mb-4"><ShellIcon size={32}/></div><span className="text-xs font-bold animate-pulse">SYNCING...</span></div>:(view==='month'&&<MonthView/>)||(view==='week'&&<WeekView/>)||(view==='day'&&<DayView/>)}</div>
    </div>
  );
};
